...
>
> > - views 안에 error 페이지를 보여주는 jsp 파일을 만들고 상단 page 태그에 아래와 같은 코드를 넣어주면 기본 error 페이지로 설정이 가능함.
> > ~~~jsp
> > <%@ page contentType="text/html;charset=utf-8" isErrorPage="true" %>
> > ~~~
> > - 위 코드는 '이 페이지는 에러가 발생했을 때 보여주기 위한 페이지'라는 의미.
> > - 위와 같이 작성하면 Model에 예외 객체를 저장해서 view에 전달할 필요없이 바로 에러 페이지에서 예외 객체를 사용할 수 있음(m.addAttribute("e", e) 메서드를 작성할 필요가 없어짐).
> > - 에러 페이지에서는 ${pageContext.exception} 코드를 통해 'exception'이라는 기본 객체를 사용할 수 있음.
>
> > - 예외 처리 방법
